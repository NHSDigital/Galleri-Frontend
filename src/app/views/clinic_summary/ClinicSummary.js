...
export default class ClinicSummary extends Component {
  ...
  getClinicsFromIcbCode() {
    ...
    axios
      .get(
        `https://erg78xcxd7.execute-api.eu-west-2.amazonaws.com/dev/clinic-summary-list?participatingIcb=${this.state.icbSelected}`
      )
      .then((response) => {
        this.setState({
  ...
  componentDidMount() {
    ...
    axios
      .get(
        `https://erg78xcxd7.execute-api.eu-west-2.amazonaws.com/dev/participating-icb-list`
      )
      .then((response) => {
        this.setState({
